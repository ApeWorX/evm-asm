...
def test_forks():
    ...
    for key, value in FORK_OPCODES.items():
        ...
        evm_opcodes = [a[0] for a in attributes if not(a[0].startswith('__') and a[0].endswith('__')) and not(a[0] == 'Name')]
        ...
        for opcode, props in value.items():
            ...
            assert evm_opcodes.__contains__(mnemonic), f'mnemonic {mnemonic} not in evm_opcodes'
            # prune our list
            evm_opcodes.remove(mnemonic)
            # fetch the opcode            
            fork_opcode = getattr(fork, mnemonic)
            
            assert opcode == fork_opcode.opcode_value, f'opcode {mnemonic}\'s hex value does not match for {key}'
            assert props.gas_cost == fork_opcode.gas_cost, f'opcode {mnemonic} gas cost does not match for {key}'
        
        print(*evm_opcodes, sep = '\n')
        print(evm_opcodes.__len__())
        assert evm_opcodes.__len__() == 0, f'{key} has an opcode that is not found in py-evm'
